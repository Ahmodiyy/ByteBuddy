import 'package:flutter/material.dart';

class Pallete {
  static const Color primaryColor = Color(0xFF26914A);
  static const Color secondaryColor = Color(0xFFffffff);
  static const Color tertiaryColor = Color(0xFFE5F8EB);

  static const Color textColor = Color(0xFF030a05);
  static const Color secondaryTextColor = Color(0xFF808080);

  static const Color contrastTextColor = Color(0xFFffffff);
  static const Color secondaryContrastTextColor = Color(0xFFf2f2f2);

  static const Color buttonColor = primaryColor;
  static const Color iconBackgroundColor = Color(0xFFECFAF5);
  static const Color errorBackgroundColor = Color(0xFF2ba152);

  static const Color errorColor = Color(0xFFdf3e2f);
  static const Color secondaryErrorColor = Color(0xFFff6978);

  static const Color accentColor = Color(0xFF210F60);
  static const Color blueGreyColor = Color(0xFFf4f7fa);

  static const Color backgroundColor = tertiaryColor;

  static const Color greyColor = Color(0xFFE0E0E0);
  static const Color tealColor = Color(0xFFA0D6B4);

  //normal F8F8FB
}
